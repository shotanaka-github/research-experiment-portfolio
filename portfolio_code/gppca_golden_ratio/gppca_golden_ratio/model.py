...
class GPEPCA:
    ...
    def from_posteriors(
        ...
        model.x_list = []
        model.y_list = []
        model.task_size = int(np.asarray(posterior_mean).shape[0])
        model.params = params
        model.latent_dim = int(latent_dim)
        model.jitter = float(jitter)
        model.random_state = int(random_state)
        model.x_all = np.asarray(x_all, dtype=np.float64)
        model.basis_count = model.x_all.shape[0]
        model.kernel_matrix = _rbf_kernel(model.x_all, model.x_all, model.params.length)
        model.kernel_matrix += model.jitter * np.eye(model.basis_count)
        model.kernel_inv = np.linalg.inv(model.kernel_matrix)
        model.posterior_mean = np.asarray(posterior_mean, dtype=np.float64)
        model.posterior_cov = np.asarray(posterior_cov, dtype=np.float64)
        model._initialize_latent_state()
        return model

    def _initialize_latent_state(self) -> None:
