...
def main(name_list_file):
    ...
    for i in range(len(name_list)):
        ...
        result_dir = "./result/50/" + name_list[i] + "/"
        ...
        # 各実験参加者の予測平均の算出
        mu = predict_mean(result=result[:100], response=response[:50])
...
def main_all(name_list_file, session):
    ...
    result_dir = "./result/all/50/" + str(session) + "/"
    if not os.path.isdir(result_dir):
        os.makedirs(result_dir)
    ...
    # 1人目の予測平均のリストの読み込み
    result_list_0 = torch.load("./result/50/" + name_list[0] + "/" + name_list[0] + "_mu.pt")

    # 1人目と2人目の予測平均のリストの結合
    result_list = torch.hstack((result_list_0, torch.load("./result/50/" + name_list[1] + "/" + name_list[1] + "_mu.pt")))
    # result_listと3人目以降の予測平均のリストの結合
    for i in range(2, len(name_list)):
        result_list = torch.hstack(
            (result_list, torch.load("./result/50/" + name_list[i] + "/" + name_list[i] + "_mu.pt")))
