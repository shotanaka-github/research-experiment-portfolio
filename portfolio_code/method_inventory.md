...
- `gp_ordinal_regression`
  - GPflow による ordinal GP と GPPCA を組み合わせた順序回帰版

## サーストン二肢選択について

- 明示的に `Thurstone` モデルとして実装された推定コードは、このワークスペース近傍では確認できなかった
- 現時点で公開版に入っている二肢選択データ関連コードは `pairwise_gppca` の Thurstone 系アダプタである
- Thurstone 推定器そのものを単独実装したコードは、このワークスペース近傍では確認できなかった
