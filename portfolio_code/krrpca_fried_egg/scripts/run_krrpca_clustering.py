...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--best-params-file", type=Path, default=None)
...
def load_hyperparameters(args: argparse.Namespace) -> KernelRidgePCAParameters:
    if args.best_params_file is None:
        return KernelRidgePCAParameters(length=args.length, noise_level=args.noise_level)

    with args.best_params_file.open("r", encoding="utf-8") as handle:
        params = json.load(handle)
    return KernelRidgePCAParameters(
...
def main() -> None:
    ...
    params = load_hyperparameters(args)
