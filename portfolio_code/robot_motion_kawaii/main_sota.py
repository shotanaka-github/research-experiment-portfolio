...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fit the robot motion preference model and export visualizations."
    )
    parser.add_argument("--data-dir", type=Path, default=Path("data"))
    parser.add_argument("--output-dir", type=Path, default=Path("result_fixed_params"))
    parser.add_argument("--model-dim", type=int, default=20)
    parser.add_argument("--length", type=float, default=DEFAULT_PARAMS["length"])
    parser.add_argument("--noise-level", type=float, default=DEFAULT_PARAMS["noise_level"])
    parser.add_argument("--cv-results", type=Path, default=None)
    parser.add_argument("--jitter", type=float, default=1e-6)
    parser.add_argument("--plot-dim", type=float, default=1.0)
